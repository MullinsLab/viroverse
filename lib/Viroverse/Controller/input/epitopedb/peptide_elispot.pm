...
package Viroverse::Controller::input::epitopedb::peptide_elispot;
...
sub index : Private {
    ...
    $context->stash->{template} = 'epitopedb/peptide_elispot.tt';
}
...
sub result : Local {
    ...
    if ($upload) {
        ...
        my $i = 0;
        my $validate_flag = 1;
        my ($exist, $update, $new, $total, $lineno) = (0) x 5;
        ...
        foreach my $line (@$lines_ref) {
            ...
            unless ($line =~ /^Exp/i || $line =~ /^\s*$/) {
                ...
                if ($pept_name || $pept_seq) {
                    if (!$exp_date || !$plate_num || !$cohort || !$patient || !$sample_date || !$tissue_type || !$cell_num || !@sfcs) {
                        $c->stash->{status} = "Missing field(s), line $lineno.";
                        $validate_flag = 0;
                        last;
                    }
                }else {
                    $c->stash->{status} = "Missing one of peptide name or sequence, line $lineno.";
                    $validate_flag = 0;
                    last;
                }
                ...
                $records->{$i} = ();
                push @{$records->{$i}}, $exp_id, $pept_id, $sample_id, $cell_num, @sfcs;
                $i++;
            }
        }
        if ($validate_flag) {
            foreach my $row (sort {$a <=> $b} keys %$records) {
                ...
                my ($measure_id, $status) = Viroverse::epitopedb::input::get_pept_elispot_measure_id($schema, $exp_id, $sample_id, $cell_num, $pept_id, \@sfcs);
                ...
                if ($status eq "exist") {
                    ...
                    if ($status->{cell}) {
                        Viroverse::epitopedb::input::update_cell_num($schema, "pept_response", $measure_id, $cell_num);
                    }
                    $update++;
                }
                $total++;
        ...
        $c->stash->{template} = 'epitopedb/peptide_elispot.tt';
    }else {
